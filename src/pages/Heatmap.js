...
const Heatmap = () => {
  ...
  useEffect(() => {
    if (!venueId) return;
    const feedbackChannel = supabase.channel('feedback-heatmap')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'feedback', filter: `venue_id=eq.${venueId}` }, () => fetchFeedback(venueId)).subscribe();
    const tablesChannel = supabase.channel('tables-heatmap')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'table_positions', filter: `venue_id=eq.${venueId}` }, () => loadTables(venueId)).subscribe();
    return () => { supabase.removeChannel(feedbackChannel); supabase.removeChannel(tablesChannel); };
  }, [venueId]);
  ...
  const handleDragStop = (e, data, tableId) => {
    const x = snapToGrid(data.x), y = snapToGrid(data.y);
    setTables(prev => prev.map(t => t.id === tableId ? { ...t, x_px: x, y_px: y } : t));
    setHasUnsavedChanges(true);
  };

  const clearAllTables = async () => {
    if (!venueId) return;
    if (!window.confirm('Are you sure you want to delete ALL tables? This cannot be undone.')) return;
            ...
            const node = (
              <div className="absolute">
                <div className="relative">
                  ...
                  {!editMode && (
                    <div className={`absolute -top-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${feedbackColor} ${feedbackColor === 'bg-red-600' ? 'animate-pulse' : ''}`} title={avgRating == null ? 'No feedback yet' : `Avg rating: ${avgRating.toFixed(1)}`} />
                  )}
                </div>
                {editMode && (
